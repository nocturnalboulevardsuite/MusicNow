...
# 2. Inicializar buscador de YouTube Music
@st.cache_resource
def get_ytmusic():
    try:
        return YTMusic()
...
# 3. ESTADO GLOBAL COMPARTIDO (Host + Invitados)
@st.cache_resource
def get_global_party_state():
    return {
        "playlist": [],
        "current_index": -1
    }

global_state = get_global_party_state()

# Cooldown individual por sesión (cada teléfono tiene su propia espera de 5 minutos)
...
COOLDOWN_SECONDS = 300  # 5 minutos de espera por usuario
...
st.markdown(
    "<p class='minimal-sub-wave'>Modo Fiesta — Las canciones añadidas se sincronizan en vivo</p>", 
    unsafe_allow_html=True
)
...
def reproducir_indice(idx):
    if 0 <= idx < len(global_state["playlist"]):
        global_state["current_index"] = idx

def siguiente_cancion():
    if global_state["playlist"] and global_state["current_index"] < len(global_state["playlist"]) - 1:
        global_state["current_index"] += 1

def anterior_cancion():
    if global_state["playlist"] and global_state["current_index"] > 0:
        global_state["current_index"] -= 1

def agregar_a_playlist(song):
    global_state["playlist"].append(song)
    if global_state["current_index"] == -1:
        global_state["current_index"] = 0

def eliminar_de_playlist(idx):
    if 0 <= idx < len(global_state["playlist"]):
        global_state["playlist"].pop(idx)
        if len(global_state["playlist"]) == 0:
            global_state["current_index"] = -1
        elif global_state["current_index"] >= len(global_state["playlist"]):
            global_state["current_index"] = len(global_state["playlist"]) - 1

# ---------------------------------------------------------
# FRAGMENTO DE TIEMPO REAL (Refresco automático cada 3s)
# ---------------------------------------------------------
@st.fragment(run_every="3s")
def render_live_party_zone():
    col_main, col_queue = st.columns([1.55, 1.05], gap="large")

    cancion_actual = None
    if 0 <= global_state["current_index"] < len(global_state["playlist"]):
        cancion_actual = global_state["playlist"][global_state["current_index"]]
    ...
    with col_main:
        ...
        components.html(html_reproductor, height=altura_componente)

        # Botones de navegación
        if global_state["playlist"]:
            col_prev, col_info, col_next = st.columns([0.35, 0.3, 0.35], vertical_alignment="center")
            with col_prev:
                st.button("⏮️ Anterior", on_click=anterior_cancion, disabled=(global_state["current_index"] <= 0), use_container_width=True)
            with col_info:
                st.markdown(f"<div style='text-align:center; font-size:0.85rem; color:#a0a0a0;'>{global_state['current_index'] + 1} de {len(global_state['playlist'])}</div>", unsafe_allow_html=True)
            with col_next:
                st.button("Siguiente ⏭️", on_click=siguiente_cancion, disabled=(global_state["current_index"] >= len(global_state["playlist"]) - 1), use_container_width=True)

    with col_queue:
        st.markdown("### Lista de espera (En vivo)")
        
        if not global_state["playlist"]:
            st.markdown(
                """
            ...
            for idx, item in enumerate(global_state["playlist"]):
                es_actual = (idx == global_state["current_index"])
            ...
            if st.button("🗑️ Limpiar lista", use_container_width=True):
                global_state["playlist"] = []
                global_state["current_index"] = -1
                st.rerun()

# Ejecutar el fragmento de sincronización en tiempo real
render_live_party_zone()

# ---------------------------------------------------------
# FORMULARIO DE BÚSQUEDA Y AGREGAR CANCIONES
# ---------------------------------------------------------
...
st.markdown("""
    <div class='qr-container'>
        ...
        <p style='color:#cccccc; font-size:0.95rem; margin-top:0;'>Apunta con la cámara de tu teléfono para entrar a la app. (1 canción cada 5 minutos por persona)</p>
    </div>
""", unsafe_allow_html=True)
...
with col_qr_center:
    url_app = st.text_input(
        ...
        value="https://musicnowcl.streamlit.app/", 
        help="Dirección URL donde está alojada tu app en Streamlit.",
        key="app_url_input"
    )
